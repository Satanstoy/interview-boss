...
- `AuthenticatedLayout.vue` 持有 `usePracticeDecks()` 的共享实例；刷题相关路由的全局 `SiteHeader` 显示“刷题 + 题单选择”，题单下拉底部提供管理入口，`PracticeMode.vue` 不再重复渲染子页面顶栏。
- 系统题单为“今日复习/全部题/我的收藏”，用户创建的自定义题单通过题卡“加入题单”和 `/practice/decks` 管理；创建弹窗提供常见八股分组建议。`/practice` 默认进入“今日复习”（due）队列，复习（记得了/很熟）后卡片移出内存队列并补偿索引避免跳卡（必须用数组替换而非 splice，否则计算链不失效、补偿 watch 不触发）。
- 刷题页顶部展示招聘状态行（`PracticeView.vue`）：当前机会窗口 + 阶段徽标（攻坚/冲刺/准备/从容）+ 距下一窗口高峰天数 + 容量与节奏（pace），数据来自 `GET /api/profile/recruitment`；未设届次（社招）时不渲染，改为「持续准备中」逻辑。设置页“面试时间偏好”（`SettingsInterview.vue`）维护届次+批次+每日容量+复习节奏（轻松/标准/冲刺）。
- `PracticeMode.vue` 的侧栏只展示当前题单的搜索框和题目列表；题卡支持频率、熟练度、收藏、加入题单，卡片内部独立滚动且题目字号随卡片容器尺寸自适应。复习排序由后端算法完成（到期复习优先 + 频率×遗忘度风险加权 + 新题容量预算），高频权重会参与所有题单队列排序；`usePracticeDecks()` 会缓存已加载题单并避免路由切换重复请求。
