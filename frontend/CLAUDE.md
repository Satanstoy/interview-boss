...
- 刷题页顶部展示招聘状态行（`PracticeView.vue`）：距最近里程碑天数 + 阶段徽标（攻坚/冲刺/准备/从容）+ 批次与每日容量，数据来自 `GET /api/profile/recruitment`；未设置批次时不渲染。设置页“面试时间偏好”（`SettingsInterview.vue`）维护届次+批次+每日容量。
