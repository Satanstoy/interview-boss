...
test.describe('今日复习默认入口与招聘状态行', () => {
  test('默认选中今日复习题单并渲染距里程碑倒计时与阶段徽标', async ({ page }) => {
    ...
    await expect(page.getByTestId('practice-plan-toggle')).toHaveAttribute('aria-expanded', 'false')
    await expect(page.getByTestId('practice-plan-details')).not.toBeVisible()
    await page.getByTestId('practice-plan-toggle').click()
    await expect(page.getByTestId('practice-plan-toggle')).toHaveAttribute('aria-expanded', 'true')
    await expect(page.getByTestId('practice-plan-details')).toBeVisible()
