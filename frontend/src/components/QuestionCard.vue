<template>
  ...
  >
    ...
    <div v-if="question._showAnswer" class="border-t border-gray-100 dark:border-gray-700 bg-gradient-to-b from-gray-50/80 to-white dark:from-surface-700/80 dark:to-surface-800 p-6 relative group answer-section">
      <!-- Source tracing -->
      <div v-if="question.sources && question.sources.length > 0" class="mb-5 bg-primary-50/40 dark:bg-primary-900/20 border border-primary-100 dark:border-primary-800/50 rounded-xl p-4">
        <h4 class="text-sm font-bold text-primary-800 dark:text-primary-300 mb-2.5 flex items-center gap-1.5">
        ...
        <div class="flex flex-wrap gap-2 text-xs">
          <span v-for="(src, idx) in question.sources" :key="idx" class="bg-white dark:bg-surface-800 border border-primary-200 dark:border-primary-700/50 text-primary-700 dark:text-primary-400 px-2.5 py-1.5 rounded-lg inline-flex items-center shadow-sm">
            {{ src.company === '未提供' ? '未知' : src.company }}
            <span class="text-primary-300 dark:text-primary-600 mx-1.5">|</span>
            {{ src.round === '未提供' ? '未知轮次' : src.round }}
            <a v-if="src.url && src.url !== '未提供链接'" :href="src.url" target="_blank" class="ml-2 text-primary-500 hover:text-primary-700 dark:hover:text-primary-300 font-bold transition-colors" title="查看原文">
              [原文]
            </a>
          </span>
        </div>
      ...
      <div v-else>
        <div v-if="question.ai_answer && !isFailedAnswer(question.ai_answer)" class="relative">
          ...
          <div class="text-gray-700 dark:text-gray-300 text-sm leading-relaxed max-w-none answer-content pt-6" v-html="cachedMarkdown"></div>
        </div>
...
defineEmits(['toggle-answer', 'toggle-star', 'retag', 'generate-answer', 'save-field', 'toggle-item', 'practice'])
