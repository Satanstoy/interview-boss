<template>
  <div class="group">
    ...
    <div v-if="!editing" class="flex items-center gap-2">
      ...
      <button @click="startEdit" class="text-gray-400 hover:text-blue-500 dark:hover:text-blue-400 opacity-0 group-hover:opacity-100 transition shrink-0 text-xs" title="编辑">
    ...
    <div v-else class="flex flex-col gap-1 w-full">
      <div class="flex items-center gap-1">
        <input v-if="type === 'text'" v-model="editValue" class="border border-gray-300 dark:border-gray-600 rounded px-2 py-1 w-full text-sm bg-white dark:bg-surface-900 text-gray-800 dark:text-gray-200" @keyup.enter="save" />
        <textarea v-else-if="type === 'textarea'" v-model="editValue" :rows="rows || 3" class="border border-gray-300 dark:border-gray-600 rounded px-2 py-1 w-full text-sm bg-white dark:bg-surface-900 text-gray-800 dark:text-gray-200"></textarea>
        <select v-else-if="type === 'select'" v-model="editValue" class="border border-gray-300 dark:border-gray-600 rounded px-2 py-1 text-sm bg-white dark:bg-surface-900 text-gray-800 dark:text-gray-200">
          <option value="">未提供</option>
          <option v-for="opt in options" :key="opt" :value="opt">{{ opt }}</option>
        </select>
        <input v-else v-model="editValue" class="border border-gray-300 dark:border-gray-600 rounded px-2 py-1 w-full text-sm bg-white dark:bg-surface-900 text-gray-800 dark:text-gray-200" @keyup.enter="save" />
