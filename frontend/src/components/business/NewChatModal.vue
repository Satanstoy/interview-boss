  ...
  <AppDialog :open="visible" size="md" @update:open="emit('close')">
    <div>
          ...
          <div class="p-6 flex flex-col gap-5">
            <!-- Mode selection -->
            <div>
              <label class="text-xs font-semibold text-muted-foreground mb-2.5 block">选择面试模式</label>
              <div class="grid grid-cols-2 gap-3">
                <button @click="mode = 'jd_resume'"
                  ...
                  <div class="text-sm font-semibold text-foreground mb-1">JD + 简历定制</div>
                  <div class="text-xs text-muted-foreground">上传目标岗位 JD 和简历，AI 针对性提问</div>
                ...
                <button @click="mode = 'free_practice'"
                  ...
                  :class="mode === 'free_practice'
                    ? 'border-primary-500 dark:border-primary-400 bg-primary-50/50 dark:bg-primary-900/20'
                    : 'border-border hover:border-primary/30'">
                  <div class="text-sm font-semibold text-foreground mb-1">自由练习</div>
                  <div class="text-xs text-muted-foreground">从题库随机出题，自由练习面试</div>
            ...
            <div v-if="mode === 'jd_resume'" class="flex flex-col gap-4">
              ...
              <!-- Resume upload -->
              <div>
                <label class="text-xs font-semibold text-muted-foreground mb-1.5 block">简历</label>
                ...
                <div class="border-2 border-dashed border-border rounded-xl p-4 text-center hover:border-primary/50 transition cursor-pointer"
                  ...
                  <div v-if="!resumeFileName">
                    <svg class="size-8 mx-auto text-muted-foreground mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"/></svg>
                    <div class="text-xs text-muted-foreground">点击上传或拖拽 PDF 文件</div>
                  </div>
          ...
          <!-- Footer -->
          <div class="flex items-center justify-end gap-3 px-6 py-4 border-t border-border bg-muted/40">
            <button @click="emit('close')" class="px-4 py-2 text-sm text-muted-foreground hover:bg-muted rounded-lg transition">取消</button>
