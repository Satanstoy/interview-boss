...
| `CodingPractice.vue` | 手撕代码练习主页面（ChatView 式全屏工作台 + 题库/题单侧栏 + 编辑器 + AI 评审）；保留收藏、题单和 Prompt + Markdown 导入 |
...
- `CodingPractice.vue` 与 `/chat` 的 `ChatView.vue` 对齐：复用全屏 `bg-background` 工作台、16rem 会话侧栏、侧栏折叠/移动端 overlay、居中主区宽度、标题栏和底部 composer 节奏；题库/题单是手撕代码对应的会话导航，进入题目后展示描述、编辑器和 AI 评审。
