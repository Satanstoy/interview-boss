  ...
  >
    ...
    <div class="p-4 flex gap-3 items-start cursor-pointer hover:bg-muted/40 dark:hover:bg-muted/20 transition-colors duration-200" @click="$emit('toggle-answer', question)">
      ...
      <button @click.stop="$emit('toggle-star', question)" class="p-1 transition-all duration-200 hover:scale-110 star-btn shrink-0" :title="question.is_starred ? '取消收藏' : '收藏'">
        <svg class="w-4.5 h-4.5 transition-colors" :class="question.is_starred ? 'text-amber-400' : 'text-border dark:text-ink-700 hover:text-amber-300 dark:hover:text-amber-500'" :fill="question.is_starred ? 'currentColor' : 'none'" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
      ...
      <div class="text-muted-foreground dark:text-ink-600 mt-0.5 shrink-0">
        <svg class="w-4 h-4 transform transition-transform duration-200" :class="question._showAnswer ? 'rotate-180 text-primary dark:text-primary' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
      </div>
    </div>
