      ...
      <div class="flex flex-col gap-6">
          ...
          <div v-if="myLLM.editing" class="flex flex-col gap-3">
            <div>
              ...
              <div v-if="myLLM.settings.llm_api_key_set && !myLLM.editKey" class="flex items-center gap-2">
                <span class="flex-1 border border-input rounded-md px-3 py-2 text-sm shadow-xs font-mono bg-surface-50 dark:bg-surface-900 text-ink-500 dark:text-ink-400 select-none">{{ myLLM.settings.llm_api_key }}</span>
            ...
            <div>
              <Label class="text-xs font-semibold text-ink-600 dark:text-ink-400 mb-1.5">Base URL <span class="text-red-400">*</span></Label>
              <Input v-model="myLLM.form.llm_base_url" type="text" placeholder="https://api.openai.com/v1" class="font-mono" />
            </div>
            <div>
              <Label class="text-xs font-semibold text-ink-600 dark:text-ink-400 mb-1.5">模型名称 <span class="text-red-400">*</span></Label>
              <Input v-model="myLLM.form.llm_model" type="text" placeholder="如 gpt-4o" class="font-mono" />
            </div>
            <div>
              <Label class="text-xs font-semibold text-ink-600 dark:text-ink-400 mb-1.5">超时 (秒)</Label>
                ...
                <div v-else class="flex flex-col gap-3">
                  ...
                  >
                    <div class="flex items-center justify-between mb-2">
                      <h4 class="text-sm font-semibold text-ink-800 dark:text-ink-100">{{ tax.position_name }}</h4>
        ...
        <template v-if="isAdmin">
          ...
          <div class="flex flex-col gap-3.5 p-5 rounded-xl border border-border bg-card">
            ...
            <div>
              <Label class="text-xs font-semibold text-ink-600 dark:text-ink-400 mb-1.5">当前招聘季</Label>
