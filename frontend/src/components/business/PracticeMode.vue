    ...
    <Card v-if="currentQ" data-testid="practice-card" class="practice-card mx-auto flex min-h-0 w-full flex-1 flex-col overflow-hidden rounded-2xl p-0 shadow-sm">
      ...
      <div data-testid="practice-card-content" :key="currentQ.id" class="flex min-h-0 min-w-0 flex-1 flex-col overflow-y-auto px-4 py-5 custom-scrollbar question-content-enter sm:px-6 md:px-12 md:py-7">
        ...
        <div v-if="showSelfCheck" class="mt-4 rounded-xl border border-primary/20 bg-primary/5 p-4 md:p-5">
          ...
          <div v-if="qState._evaluation" class="mt-4 rounded-lg border border-border bg-card p-4"><div class="flex items-center gap-3"><span class="text-2xl font-bold" :class="scoreTextColor(qState._evaluation.overall_score)">{{ qState._evaluation.overall_score }}</span><div class="min-w-0 flex-1"><div class="h-2 overflow-hidden rounded-full bg-muted"><div class="h-full rounded-full transition-all" :class="scoreColor(qState._evaluation.overall_score)" :style="{ width: `${qState._evaluation.overall_score}%` }"></div></div><p class="mt-1 text-[11px] text-muted-foreground">{{ evaluationSummary(qState._evaluation) }}</p></div></div><p v-if="qState._evaluation.truncated" data-testid="practice-eval-truncated" class="mt-2 text-[11px] text-amber-600 dark:text-amber-400">你的回答已超长，评估基于前 3000 字符，可精简后重新评估。</p></div>
        </div>
