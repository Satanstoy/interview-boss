<template>
  <div class="fixed inset-0 z-[90] bg-background flex flex-col">
    <!-- TOP BAR -->
    ...
    <Transition name="directory-slide">
      <div v-if="showDirectory" class="absolute inset-y-12 left-0 z-10 w-80 bg-card border-r border-border shadow-xl flex flex-col">
        <div class="p-3 border-b border-border flex items-center gap-2">
          <svg class="size-4 text-muted-foreground shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
          <input v-model="directorySearch" type="text" class="flex-1 text-sm bg-transparent outline-none text-foreground placeholder-muted-foreground" placeholder="搜索题目..." />
          <button @click="showDirectory = false; directorySearch = ''" class="p-1 rounded text-muted-foreground hover:text-muted-foreground dark:hover:text-muted-foreground/50 transition">
        ...
        <div ref="directoryRef" class="flex-1 overflow-y-auto custom-scrollbar">
          <button
            ...
            class="w-full text-left px-3 py-2.5 border-b border-border/50 hover:bg-muted dark:hover:bg-muted transition-colors"
            :class="item.id === currentQ.id ? 'bg-primary-50 dark:bg-primary-900/20 border-l-2 border-l-primary-500' : ''"
          >
            <div class="flex items-center gap-2 mb-1">
              <span class="text-[10px] font-bold tabular-nums w-5 text-center" :class="item.id === currentQ.id ? 'text-primary-600 dark:text-primary-400' : 'text-muted-foreground'">{{ questions.indexOf(item) + 1 }}</span>
              ...
              <span class="text-[10px] text-muted-foreground ml-auto tabular-nums">{{ item.frequency }}</span>
            </div>
            <p class="text-xs text-foreground leading-snug line-clamp-2 ml-7">{{ item.question }}</p>
          </button>
        </div>
      </div>
    </Transition>
    ...
    <div ref="mainRef" class="relative flex-1 overflow-hidden"
      ...
      <!-- LEFT PANEL -->
      <div class="flex flex-col overflow-hidden border-b lg:border-b-0 lg:border-r border-border min-w-0">
        <!-- Tabs -->
        <Tabs default-value="description" v-model:value="leftTab">
          <TabsList class="flex border-b border-border shrink-0 bg-card rounded-none">
            ...
            <TabsTrigger value="history">
              练习记录
              <span v-if="currentQ.attempt_count" class="ml-1 text-[10px] text-muted-foreground">({{ currentQ.attempt_count }})</span>
            </TabsTrigger>
          </TabsList>
        ...
        <div :key="currentIndex" class="question-content-enter">
          ...
          <TabsContent value="description">
            ...
            <div class="flex gap-1.5 flex-wrap items-center">
              ...
              <Badge v-if="currentQ.cat2" variant="outline" class="bg-muted dark:bg-card text-muted-foreground border-border text-[10px]">{{ currentQ.cat2 }}</Badge>
              <Badge v-for="tag in (currentQ.tags ? currentQ.tags.split(',') : [])" :key="tag" variant="outline" class="bg-muted dark:bg-background text-muted-foreground border-border/60 text-[10px]">{{ tag }}</Badge>
            </div>
            <!-- Question text -->
            <div class="text-sm text-foreground leading-relaxed font-medium">{{ currentQ.question }}</div>
            <!-- Original question content -->
            <div v-if="currentQ.original_questions && currentQ.original_questions.length > 0" class="bg-muted dark:bg-background border border-border rounded-xl p-4">
              ...
              <div v-for="(oq, oidx) in currentQ.original_questions" :key="oidx" class="mb-2 last:mb-0">
                <p class="text-xs text-foreground leading-relaxed">{{ oq.question || oq }}</p>
            ...
            <div v-if="currentQ.sources && currentQ.sources.length > 0" class="bg-primary-50/40 dark:bg-primary-900/20 border border-primary-100 dark:border-primary-800 rounded-xl p-4">
              ...
              <div class="flex flex-wrap gap-1.5 text-[11px]">
                <span v-for="(src, idx) in currentQ.sources" :key="idx" class="bg-card border border-primary-200 dark:border-primary-800 text-primary-700 dark:text-primary-400 px-2 py-1 rounded-lg inline-flex items-center gap-1">
          ...
          <TabsContent value="answer">
            ...
            <div v-else-if="currentQ.ai_answer && !isFailedAnswer(currentQ.ai_answer)">
              ...
              <div class="text-sm text-foreground leading-relaxed answer-content" v-html="renderMarkdown(currentQ.ai_answer)"></div>
            </div>
          ...
          <TabsContent value="history">
            ...
            <div v-else-if="qState._history && qState._history.length > 0" class="flex flex-col gap-2">
              <div v-for="(h, hIdx) in qState._history" :key="h.id" v-auto-animate class="border border-border rounded-xl overflow-hidden">
      ...
      <div v-if="!isMobile" class="bg-muted dark:bg-muted hover:bg-primary-400 dark:hover:bg-primary-500 cursor-col-resize transition-colors relative group" @mousedown="onDividerMouseDown">
        <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-1 h-8 rounded-full bg-muted dark:bg-muted group-hover:bg-primary-500 transition-colors"></div>
      </div>
      ...
      <div class="flex flex-col overflow-hidden min-h-0 min-w-0">
        ...
        <div class="flex-1 flex flex-col overflow-hidden min-h-0" :style="qState._evaluation && consoleExpanded ? {} : { flex: 1 }">
          ...
          <div class="px-5 pb-3 flex gap-2 items-center shrink-0">
            ...
            <span class="text-[10px] text-muted-foreground hidden sm:inline">Ctrl+Enter</span>
            <button v-if="qState._userAnswer" @click="qState._userAnswer = ''; qState._evaluation = null"
              class="text-sm text-muted-foreground px-3 py-2 rounded-lg border border-border hover:bg-muted dark:hover:bg-muted transition">清空</button>
          </div>
        </div>

        <!-- Console panel (evaluation results) -->
        <div v-if="qState._evaluation" v-auto-animate class="border-t border-border shrink-0 flex flex-col" :style="consoleExpanded ? { maxHeight: '45%' } : {}">
          <!-- Console header -->
          <button @click="consoleExpanded = !consoleExpanded" class="flex items-center justify-between px-5 py-2.5 bg-muted dark:bg-background hover:bg-muted dark:hover:bg-muted transition shrink-0">
            <div class="flex items-center gap-3">
              <span class="text-xs font-bold text-muted-foreground">评估结果</span>
            ...
            <svg class="size-4 text-muted-foreground transition-transform" :class="{ 'rotate-180': consoleExpanded }" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7"/></svg>
          </button>
