    ...
    <Transition name="fade">
      <div v-if="visible" class="fixed inset-0 z-[9998] flex items-center justify-center bg-black/50 backdrop-blur-sm" @click.self="$emit('close')">
        <div class="bg-white dark:bg-surface-800 rounded-3xl shadow-2xl w-full max-w-2xl mx-4 max-h-[80vh] flex flex-col animate-slide-up overflow-hidden">
          <!-- Header -->
          <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between bg-gradient-to-r from-orange-50/50 to-amber-50/30 dark:from-orange-900/20 dark:to-amber-900/10">
            <div class="flex items-center gap-3">
              ...
              <div>
                <h2 class="text-lg font-bold text-gray-800 dark:text-gray-100">待审核题目</h2>
                <p class="text-xs text-gray-400 dark:text-gray-500 mt-0.5">审核用户提交到公共题库的题目</p>
              </div>
            </div>
          ...
          <div class="flex-1 overflow-y-auto custom-scrollbar px-6 py-4">
            ...
            <div v-if="loading" class="flex flex-col items-center justify-center py-16 gap-3">
              ...
              <span class="text-gray-400 dark:text-gray-500 text-sm">加载中...</span>
            </div>
            ...
            <div v-else-if="items.length === 0" class="text-center py-16">
              ...
              <p class="text-gray-500 dark:text-gray-400 font-medium">暂无待审核题目</p>
              <p class="text-sm text-gray-400 dark:text-gray-500 mt-1">所有题目均已审核完毕</p>
            </div>
            ...
            <div v-else v-auto-animate class="space-y-3">
              <div
                v-for="item in items"
                :key="item.id"
                class="border border-gray-100 dark:border-gray-700 rounded-xl p-4 hover:border-primary-200 dark:hover:border-primary-700 hover:shadow-sm transition-all duration-200 animate-fade-in"
              >
                <div class="flex items-start justify-between gap-3">
                  <div class="flex-1 min-w-0">
                    <p class="text-sm font-medium text-gray-800 dark:text-gray-100 leading-relaxed">{{ item.question }}</p>
                    <div class="flex flex-wrap gap-1.5 mt-2.5">
                      ...
                      <span v-if="item.submitted_by_name" class="badge bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400">{{ item.submitted_by_name }}</span>
                    </div>
                  </div>
          ...
          <!-- Footer -->
          <div class="px-6 py-3.5 border-t border-gray-100 dark:border-gray-700 flex justify-between items-center bg-gray-50/80 dark:bg-surface-900/80">
            <span class="text-sm text-gray-500 dark:text-gray-400">共 <span class="font-bold text-gray-700 dark:text-gray-300">{{ items.length }}</span> 条待审核</span>
