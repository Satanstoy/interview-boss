...
function toolSummary(item) {
  const value = metrics(item).tool_metrics
  if (!value) return '—'
  return `${value.call_count || 0} 次 · ${value.failed_call_count || 0} 失败`
}

function intentSummary(item) {
  const value = metrics(item).intent_metrics
  if (!value) return '—'
  return `${value.evaluated_count || 0} 次 · ${value.failed_count || 0} 失败`
}
    ...
    <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
      ...
      <template v-else-if="run">
        ...
        <AppCard class="mt-6" title="逐 Case 结果" description="先看执行状态和门禁结果；完整 transcript、工具轨迹和 Judge 原文在单个 Artifact 中查看。" no-padding>
          <div class="overflow-x-auto"><table class="w-full min-w-[1040px] text-left text-sm"><thead class="border-b border-border/60 bg-muted/30 text-xs text-muted-foreground"><tr><th class="px-5 py-3">Case / 重跑</th><th class="px-5 py-3">执行状态</th><th class="px-5 py-3">契约</th><th class="px-5 py-3">工具调用</th><th class="px-5 py-3">意图识别</th><th class="px-5 py-3">硬门禁</th><th class="px-5 py-3">Judge</th><th class="px-5 py-3">分数</th></tr></thead><tbody class="divide-y divide-border/60"><tr v-for="item in run.items" :key="item.id" class="hover:bg-muted/20"><td class="px-5 py-3"><div class="font-medium">{{ item.case_key }}</div><div class="text-xs text-muted-foreground">第 {{ item.replication_index }} 次 · seed {{ item.seed }}</div></td><td class="px-5 py-3"><span :class="['rounded-full px-2 py-0.5 text-xs', statusClass(item.status)]">{{ statusLabel(item.status) }}</span></td><td class="px-5 py-3 text-xs">{{ checkStatusLabel(item.contract_status) }}</td><td class="px-5 py-3 text-xs"><div>{{ toolSummary(item) }}</div><div class="text-[11px] text-muted-foreground">工具轨迹</div></td><td class="px-5 py-3 text-xs"><div>{{ intentSummary(item) }}</div><div class="text-[11px] text-muted-foreground">分类与策略</div></td><td class="px-5 py-3 text-xs">{{ checkStatusLabel(item.hard_gate_status) }}</td><td class="px-5 py-3 text-xs">{{ checkStatusLabel(item.judge_status) }}</td><td class="px-5 py-3 font-mono">{{ item.score == null ? '—' : Number(item.score).toFixed(3) }}</td></tr></tbody></table></div>
