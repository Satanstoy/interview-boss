        ...
        <AppCard v-for="group in groups" :key="group.type" :title="group.title" :description="group.description" no-padding>
          <div class="overflow-x-auto">
            <table class="w-full table-fixed text-left text-sm">
              <thead class="border-b border-border/60 bg-muted/30 text-xs text-muted-foreground"><tr><th class="w-[28%] px-5 py-3 font-medium">版本</th><th class="w-[12%] px-5 py-3 font-medium">状态</th><th class="w-[32%] px-5 py-3 font-medium">关键固定项</th><th class="w-[20%] px-5 py-3 font-medium">创建时间</th><th class="w-[8%] px-5 py-3 font-medium"></th></tr></thead>
              <tbody class="divide-y divide-border/60">
                <template v-for="release in group.items" :key="release.id">
                  <tr class="hover:bg-muted/20">
                    <td class="align-top px-5 py-4"><div class="flex min-w-0 items-start gap-2 font-medium"><GitBranch class="mt-0.5 size-4 shrink-0 text-primary" /><span class="break-words">{{ release.release_key }}</span></div><div class="mt-1 text-xs text-muted-foreground">版本 {{ release.version }} · {{ releaseTypeLabel(release.release_type) }}</div></td>
                    <td class="align-top px-5 py-4"><span :class="['inline-block rounded-full px-2 py-0.5 text-xs', statusClass(release.status)]">{{ statusLabel(release.status) }}</span></td>
                    <td class="align-top break-words px-5 py-4 text-xs text-muted-foreground"><span v-if="release.judge_model" class="break-all font-mono text-foreground">{{ release.judge_model }}</span><span v-else-if="release.git_sha" class="break-all font-mono text-foreground">Git {{ release.git_sha.slice(0, 12) }}</span><span v-else-if="release.image_digest" class="break-all font-mono text-foreground">Image {{ release.image_digest.slice(0, 12) }}</span><span v-else>已记录在 Manifest</span></td>
                    <td class="align-top break-words px-5 py-4 text-xs text-muted-foreground">{{ formatDate(release.created_at) }}</td>
                    <td class="align-top px-5 py-4 text-right"><button type="button" class="rounded-md p-1.5 text-muted-foreground hover:bg-muted hover:text-foreground" :aria-label="`查看 ${release.release_key} 详情`" @click="toggle(release.id)"><ChevronDown :class="['size-4 transition-transform', opened === release.id ? 'rotate-180' : '']" /></button></td>
                  </tr>
