            ...
            <table class="w-full table-fixed text-left text-sm">
              ...
              <tbody class="divide-y divide-border/60">
                <template v-for="release in group.items" :key="release.id">
                  ...
                  <tr v-if="opened === release.id" class="bg-muted/20"><td colspan="5" class="px-5 py-4"><div class="grid gap-3 text-xs sm:grid-cols-3"><div><div class="text-muted-foreground">Manifest Digest</div><div class="mt-1 break-all font-mono text-foreground">{{ release.manifest_digest || '—' }}</div></div><div><div class="text-muted-foreground">Git SHA</div><div class="mt-1 break-all font-mono text-foreground">{{ release.git_sha || '—' }}</div></div><div><div class="text-muted-foreground">配置摘要</div><div class="mt-1 break-all font-mono text-foreground">{{ release.config_digest || '—' }}</div></div></div></td></tr>
                </template>
              </tbody>
            </table>
          </div>
