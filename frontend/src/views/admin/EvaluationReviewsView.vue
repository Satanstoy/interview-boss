          ...
          <template v-else>
            ...
            <div class="mt-6 grid gap-5 lg:grid-cols-2">
              <article v-for="entry in [{ item: itemA, label: '回答 A', run: runA }, { item: itemB, label: '回答 B', run: runB }]" :key="entry.label" class="rounded-xl border border-border/70 bg-muted/20 p-4">
                ...
                <div v-if="entry.item" class="mb-4 grid gap-2 text-xs sm:grid-cols-2"><div v-if="metrics(entry.item).tool_metrics" class="rounded-lg bg-background p-3 ring-1 ring-border/60"><div class="font-medium">工具调用效果</div><div class="mt-1 text-muted-foreground">{{ metrics(entry.item).tool_metrics?.call_count || 0 }} 次调用 · {{ metrics(entry.item).tool_metrics?.failed_call_count || 0 }} 次失败 · 结果{{ metrics(entry.item).tool_metrics?.result_used ? '已使用' : '未确认使用' }}</div></div><div v-if="metrics(entry.item).intent_metrics" class="rounded-lg bg-background p-3 ring-1 ring-border/60"><div class="font-medium">意图识别效果</div><div class="mt-1 text-muted-foreground">{{ metrics(entry.item).intent_metrics?.observed_turn_count || 0 }} 轮记录 · 覆盖率 {{ metrics(entry.item).intent_metrics?.intent_coverage == null ? '—' : `${Math.round(metrics(entry.item).intent_metrics.intent_coverage * 100)}%` }} · 准确率 {{ metrics(entry.item).intent_metrics?.accuracy == null ? '—' : `${Math.round(metrics(entry.item).intent_metrics.accuracy * 100)}%` }}</div></div><div v-if="metrics(entry.item).metrics?.field_coverage != null" class="rounded-lg bg-background p-3 ring-1 ring-border/60"><div class="font-medium">结构化抽取</div><div class="mt-1 text-muted-foreground">字段覆盖 {{ Math.round(metrics(entry.item).metrics.field_coverage * 100) }}% · 题目召回 {{ Math.round((metrics(entry.item).metrics.question_recall || 0) * 100) }}%</div></div><div v-if="metrics(entry.item).metrics?.source_fact_coverage != null" class="rounded-lg bg-background p-3 ring-1 ring-border/60"><div class="font-medium">简历事实与岗位</div><div class="mt-1 text-muted-foreground">事实覆盖 {{ Math.round(metrics(entry.item).metrics.source_fact_coverage * 100) }}% · 岗位匹配 {{ Math.round((metrics(entry.item).metrics.target_alignment || 0) * 100) }}%</div></div><div v-if="metrics(entry.item).metrics?.taxonomy_validity != null" class="rounded-lg bg-background p-3 ring-1 ring-border/60"><div class="font-medium">题目分类</div><div class="mt-1 text-muted-foreground">分类合法 {{ Math.round(metrics(entry.item).metrics.taxonomy_validity * 100) }}% · 标签准确 {{ Math.round((metrics(entry.item).metrics.classification_accuracy || 0) * 100) }}%</div></div><div class="text-xs text-muted-foreground">Hard Gate：{{ entry.item.hard_gate_status || '—' }} · Judge：{{ entry.item.judge_status || '—' }} · 分数：{{ entry.item.score == null ? '—' : Number(entry.item.score).toFixed(3) }}</div></div>
