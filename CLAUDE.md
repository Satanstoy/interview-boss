...
| 来源健康合并 | `routers/admin_source_health.py` + `services/interview_merge_service.py`（同签名重复公共面经列表/合并，仅公共 `owner_id IS NULL`）+ `services/source_health.py`（检测）+ `scripts/fix_source_consistency.py`（运维合并） | `services/adminSourceHealthApi.js` + `components/business/SettingsSourceHealth.vue`（聚合质量 tab 内「来源健康」子切换） |
