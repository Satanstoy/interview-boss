...
实现中，控制事实解释器与承担记忆召回、关键词和检索改写的 broad classifier
独立运行。它每轮只裁决 `requested_end`、`counter_question` 与 `answer_state`；
前两项优先级高于回答完整度，并要求反问文本直接摘自候选人原话。这样显式反问
或结束不会因上一题尚未作答而被 broad classifier 误标成 `incomplete`。
