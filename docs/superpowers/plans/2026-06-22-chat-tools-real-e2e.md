# Chat Tools Real E2E Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a manual production-container verification script that measures whether real chat tool calls bind retrieved/drawn questions reliably.

**Architecture:** Create one self-contained Python script under `backend/scripts/` that logs in via real HTTP, creates a chat conversation, posts SSE messages, parses events, computes stability metrics, and cleans up the conversation. Update scripts documentation and dev-log only.

**Tech Stack:** Python 3.10, stdlib `urllib.request`, Server-Sent Events parsing, existing FastAPI chat/auth endpoints, Docker backend container.

---

## Task 1: Add Real E2E Script

**Files:**
- Create: `backend/scripts/verify_chat_tools_real_e2e.py`

- [ ] **Step 1: Create script with guard, login, SSE parsing, cases, report**

Create `backend/scripts/verify_chat_tools_real_e2e.py` with this complete content:
...
Expected: exits 2 and prints `Refusing to run real LLM E2E. Set RUN_REAL_CHAT_E2E=1.`

---

## Task 2: Update docs and dev log

**Files:**
- Modify: `backend/scripts/CLAUDE.md`
- Create: `docs/dev-log/2026-06-22-chat-tools-real-e2e.md`

- [ ] **Step 1: Update `backend/scripts/CLAUDE.md`**

Add this row to the file table:
...
```bash
git add backend/scripts/verify_chat_tools_real_e2e.py backend/scripts/CLAUDE.md docs/dev-log/2026-06-22-chat-tools-real-e2e.md
git commit -m "test(backend): add real chat tools e2e verifier"
```
...
- Spec coverage: real HTTP login, conversation creation, SSE parsing, four cases, metrics, guard, cleanup, docs.
- No hardcoded secrets.
- No production DB mutation beyond creating/deleting one chat conversation.
- Script is manual and not part of pytest.
