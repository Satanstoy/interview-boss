...
1. **分配只带 top-N 相似标签**：复用字符相似度预筛，133 标签 → 只带 10-20 个相关标签 → 分配 token 降 85-90%（每轮 0.96M → ~0.15M）。这是最大杠杆
2. **标签摘要缓存持久化**：133 个标签生成一次存 DB（round_results.json 模式），增量导入/后续轮次不重生成（省 7 次/轮；R2-R6 每轮重复生成是浪费）
3. **重试上限控制**：tenacity 设 max attempts + 熔断（R6 教训：mimo 抖动时 788 次重试烧 3.56M）
4. **生产场景天然更省**：增量导入只对新题分配+验证（几题/次），不做全量 187 次重扫
