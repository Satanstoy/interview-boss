...
def create_test_db():
    ...
    """)
    conn.execute(
        "INSERT INTO users (id, username, password_hash, is_admin) VALUES (1, 'admin', 'hash', 1)"
    )
    conn.execute(
        "INSERT INTO users (id, username, password_hash, is_admin) VALUES (2, 'user', 'hash', 0)"
    )
    conn.execute("INSERT INTO job_positions (id, name) VALUES (1, '后端开发')")
    conn.execute(
        "INSERT INTO user_profile (user_id, key, value) VALUES (1, 'current_job_position', '后端开发')"
    )
    conn.execute(
        "INSERT INTO user_profile (user_id, key, value) VALUES (2, 'current_job_position', '后端开发')"
    )
    conn.commit()
    return conn
...
MOCK_MATCH_RESPONSE = json.dumps({"matches": []}, ensure_ascii=False)

MOCK_FILL_RESPONSE = json.dumps(
    {"公司": "字节跳动", "面试轮次": "二面"}, ensure_ascii=False
)
...
def _mock_tag_batch(
    url, company, round_, questions, taxonomy_config=None, user_id=None
):
    ...
    for q in questions:
        if "Redis" in q or "缓存" in q:
            results.append(
                [url, company, round_, q, "数据库", "Redis", "Redis,缓存", "L2-中等"]
            )
        elif "TCP" in q or "网络" in q or "HTTP" in q:
            results.append(
                [url, company, round_, q, "计算机网络", "TCP", "TCP,网络", "L1-基础"]
            )
        elif "算法" in q or "排序" in q or "快排" in q:
            results.append(
                [url, company, round_, q, "算法", "排序", "排序,算法", "L3-困难"]
            )
        else:
...
def _mock_tag_batch_low_quality(
    url, company, round_, questions, taxonomy_config=None, user_id=None
):
    """返回低质量分类结果（有效但错误的分类）"""
    return [
        [url, company, round_, q, "INVALID_CAT1", "INVALID_CAT2", "", "INVALID_DIFF"]
        for q in questions
    ]
...
def mock_db():
    ...
    with (
        ...
        patch(
            ...
            return_value={"matched_to_existing": [], "new_clusters": []},
        ),
        patch(
            "app.services.faiss_index_manager.get_index_manager",
...
class TestSubmitNodes:
    ...
    def test_extract_node_produces_correct_data(self, mock_db):
        ...
        with (
            patch("app.services.llm._call_llm_with_retry_messages", mock_llm),
            patch("app.services.llm._should_use_response_format", return_value=False),
            patch(
                "app.services.llm.get_llm_client_for_user",
                return_value=(MagicMock(), "test-model", 30, "", "openai"),
            ),
        ):
            state = {
                "raw_text": "面经内容...",
    ...
    def test_extract_node_blacklist_filter(self, mock_db):
        ...
        with (
            ...
            patch(
                "app.services.llm.get_llm_client_for_user",
                return_value=(MagicMock(), "model", 30, "", "openai"),
            ),
        ):
            state = {
                "raw_text": "面经",
    ...
    def test_match_and_persist_personal_creates_qb_records(self, mock_db):
        ...
        with (
            patch(
                "app.services.clustering.match_new_questions",
                AsyncMock(return_value={"matched": [], "unmatched": []}),
            ),
            patch(
                "app.db.connection.get_current_job_position", return_value="后端开发"
            ),
        ):
            state = {
                "extracted_data": {
        ...
        conn = mock_db
        iv = conn.execute(
            "SELECT * FROM interview WHERE url = 'https://test.com/1'"
        ).fetchone()
        assert iv is not None
        assert iv["company"] == "字节"

        details = conn.execute(
            "SELECT * FROM questions_detail WHERE url = 'https://test.com/1'"
        ).fetchall()
    ...
    def test_persist_public_writes_and_enqueues(self, mock_db):
        ...
        conn = mock_db
        iv = conn.execute(
            "SELECT * FROM interview WHERE url = 'https://test.com/2'"
        ).fetchone()
        assert iv is not None
...
class TestSubmitGraphRouting:
    ...
    def test_after_extract_max_retries_forces_continue(self):
        ...
        # 有题目但质量低 → 重试耗尽后继续
        state = {
            "extraction_quality": 2.0,
            "extraction_retries": 2,
            "extracted_data": {"具体题目清单": ["题1"]},
        }
        assert after_extract(state) == "continue"
...
class TestBuildNodes:
    ...
    def test_load_all_node_enqueues(self, mock_db):
        ...
        # 插入测试面经 + 题目
        conn.execute(
            "INSERT INTO interview (id, url, company, round, job_position) VALUES (100, 'https://test.com', '字节', '一面', '后端开发')"
        )
        conn.execute(
            "INSERT INTO questions_detail (id, url, company, round, question, cat1, cat2, tags, diff_tag, job_position) VALUES (200, 'https://test.com', '字节', '一面', 'Redis持久化', '数据库', 'Redis', '', 'L2-中等', '后端开发')"
        )
        conn.commit()
    ...
    def test_cluster_loop_node_processes_batches(self, mock_db):
        ...
        # 模拟队列中有数据
        conn.execute(
            "INSERT INTO interview (id, url, company, round, job_position) VALUES (101, 'https://t.com', '公司', '一面', '后端开发')"
        )
        conn.execute(
            "INSERT INTO questions_detail (id, url, company, round, question, cat1, cat2, tags, diff_tag, job_position) VALUES (301, 'https://t.com', '公司', '一面', 'Q1', 'A', 'B', '', 'L2', '后端开发')"
        )
        conn.execute(
            "INSERT INTO analysis_queue (interview_id, question_detail_id, status) VALUES (101, 301, 'pending')"
        )
        conn.commit()

        with (
            patch("app.services.pipeline.cluster_batch", AsyncMock(return_value=1)),
            patch("app.services.pipeline.mark_batch_done"),
        ):
            result = asyncio.run(
                cluster_loop_node(
                    {"user_id": 1, "total_questions": 1, "processed_count": 0}
                )
            )
...
class TestBatchGenerateNodes:
    ...
    def test_load_questions_node(self, mock_db):
        ...
        conn = mock_db
        conn.execute(
            "INSERT INTO question_bank (id, question, ai_answer) VALUES (10, 'Q1', NULL)"
        )
        conn.execute(
            "INSERT INTO question_bank (id, question, ai_answer) VALUES (11, 'Q2', '')"
        )
        conn.execute(
            "INSERT INTO question_bank (id, question, ai_answer) VALUES (12, 'Q3', '已有答案')"
        )
        conn.commit()
    ...
    def test_generate_answer_node_saves_to_db(self, mock_db):
        ...
        with patch(
            "app.services.llm._call_llm_with_retry", AsyncMock(return_value=MOCK_ANSWER)
        ):
            state = {
                "question_ids": [20],
                "current_index": 0,
        ...
        # 验证答案已写入 DB
        row = conn.execute(
            "SELECT ai_answer FROM question_bank WHERE id = 20"
        ).fetchone()
    ...
    def test_should_continue_generate(self):
        ...
        assert (
            should_continue_generate({"current_index": 2, "question_ids": [1, 2, 3]})
            == "continue"
        )
        assert (
            should_continue_generate({"current_index": 3, "question_ids": [1, 2, 3]})
            == "done"
        )
...
class TestSubmitGraphE2E:
    ...
    def test_personal_interview_e2e(self, mock_db):
        ...
        with (
            ...
            result = asyncio.run(
                submit_graph.ainvoke(
                    input_state,
                    config={"configurable": {"thread_id": "test-personal-e2e"}},
                )
            )
        ...
        conn = mock_db
        iv = conn.execute(
            "SELECT * FROM interview WHERE url = 'https://test.com/e2e'"
        ).fetchone()
        assert iv is not None
        details = conn.execute(
            "SELECT * FROM questions_detail WHERE url = 'https://test.com/e2e'"
        ).fetchall()
    ...
    def test_public_interview_e2e(self, mock_db):
        ...
        with (
            ...
            patch(
                "app.services.submit_service.tag_questions_batch",
                AsyncMock(side_effect=_mock_tag_batch),
            ),
            patch(
                "app.db.connection.get_current_job_position", return_value="后端开发"
            ),
            ...
            result = asyncio.run(
                submit_graph.ainvoke(
                    input_state,
                    config={"configurable": {"thread_id": "test-public-e2e"}},
                )
            )
        ...
        conn = mock_db
        iv = conn.execute(
            "SELECT * FROM interview WHERE url = 'https://test.com/e2e-public'"
        ).fetchone()
        assert iv is not None
    ...
    def test_classification_quality_retry(self, mock_db):
        ...
        with (
            patch(
                "app.services.submit_service.tag_questions_batch",
                AsyncMock(side_effect=_mock_tag_batch_low_quality),
            ),
            patch("app.db.connection.get_taxonomy_for_position", return_value=None),
            patch(
                "app.db.connection.run_db",
