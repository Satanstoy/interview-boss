...
def _mock_tag_batch(
    ...
    for q in questions:
        if "Redis" in q or "缓存" in q:
            ...
            results.append(
                [url, company, round_, q, "计算机网络", "TCP", "TCP,网络", "L1-简单"]
            )
...
def mock_db():
    ...
    with (
        ...
        patch(
            ...
            side_effect=_mock_process_incremental_batch,
        ),
        patch(
...
class TestSingleInterview:
    ...
    @patch(
        "app.services.submit_service.tag_questions_batch", side_effect=_mock_tag_batch
    )
    @patch(
        "app.services.pipeline.batch.cluster_batch",
        new_callable=AsyncMock,
        side_effect=_mock_cluster_all_questions,
    )
    @patch(
        "app.services.clustering.generate_unified_question",
        new_callable=AsyncMock,
        side_effect=_mock_generate_unified,
    )
    def test_full_pipeline(self, mock_uq, mock_cluster, mock_tag, mock_db):
        ...
        async def _run():
            ...
            # QD 写入了
            qd = conn.execute(
                "SELECT * FROM questions_detail WHERE url = 'https://iv1.com'"
            ).fetchall()
            assert len(qd) == 3
...
class TestBatchProcessing:
    ...
    def test_batch_merges_same_cat2(self, mock_uq, mock_cluster, mock_tag, mock_db):
        ...
        async def _run():
            ...
            assert (
                conn.execute("SELECT COUNT(*) as c FROM questions_detail").fetchone()[
                    "c"
                ]
                == 6
            )
            assert (
                conn.execute("SELECT COUNT(*) as c FROM question_bank").fetchone()["c"]
                == 0
            )
            assert should_trigger_clustering(BATCH_SIZE) is True
            ...
            pending = conn.execute(
                "SELECT COUNT(*) as c FROM analysis_queue WHERE status != 'done'"
            ).fetchone()["c"]
            assert pending == 0
...
class TestRebuildFlow:
    ...
    def test_rebuild_clears_old_and_creates_new(
        self, mock_uq, mock_cluster, mock_tag, mock_db
    ):
        ...
        async def _run():
            ...
            # ── 模拟重建 ──
            conn.execute(
                "DELETE FROM user_practice_history WHERE question_bank_id IN (SELECT id FROM question_bank WHERE owner_id IS NULL)"
            )
            conn.execute(
                "DELETE FROM user_question_view WHERE question_bank_id IN (SELECT id FROM question_bank WHERE owner_id IS NULL)"
            )
            conn.execute(
                "DELETE FROM question_position WHERE question_id IN (SELECT id FROM question_bank WHERE owner_id IS NULL)"
            )
...
class TestDataConsistency:
    ...
    def test_phase1_does_not_touch_qb(self, mock_uq, mock_cluster, mock_tag, mock_db):
        ...
        async def _run():
            ...
            conn.execute(
                "INSERT INTO question_bank (question, cat1, cat2, frequency, sources, owner_id, job_position) VALUES ('已有题', 'A', 'B', 1, '[]', NULL, '后端开发')"
            )
            conn.commit()
    ...
    def test_reprocess_no_duplicate_sources(
        self, mock_uq, mock_cluster, mock_tag, mock_db
    ):
        ...
        async def _run():
            ...
            # 第一次
            await tag_interview(
                url, "腾讯", "一面", "1. Redis 持久化？", job_position="后端开发"
            )
            enqueue_questions(1)
...
class TestCleanupThoroughness:
    def test_cleanup_removes_oqs_by_url(self):
            ...
            (
                ...
                json.dumps(
                    [
                        {
                            "question": "题目A",
                            "sources": [{"url": url, "company": "腾讯"}],
                        },
                        {
                            "question": "题目B",
                            "sources": [{"url": "https://keep.com", "company": "阿里"}],
                        },
                    ]
                ),
            ),
        ...
        conn.execute(
            "INSERT OR IGNORE INTO question_position (question_id, position_id) VALUES (100, 1)"
        )
        conn.commit()
    ...
    def test_cleanup_deletes_qb_when_all_sources_gone(self):
        ...
        conn.execute(
            "INSERT OR IGNORE INTO question_position (question_id, position_id) VALUES (200, 1)"
        )
        conn.commit()
        ...
        assert (
            len(
                conn.execute(
                    "SELECT * FROM question_position WHERE question_id = 200"
                ).fetchall()
            )
            == 0
        )
...
class TestEdgeCases:
    ...
    def test_empty_questions_list(self, mock_tag, mock_db):
        ...
        async def _run():
            ...
            tagged = await tag_interview(
                "https://empty.com", "公司", "一面", "", job_position="后端开发"
            )
            assert tagged == []
    ...
    def test_reprocess_preserves_ai_answer(
        self, mock_uq, mock_cluster, mock_tag, mock_db
    ):
        """重新聚类保留已有 AI 答案"""
        from app.services.pipeline import (
        ...
        async def _run():
            ...
            # 加 AI 答案
            qb = conn.execute(
                "SELECT id FROM question_bank WHERE owner_id IS NULL LIMIT 1"
            ).fetchone()
            conn.execute(
                "UPDATE question_bank SET ai_answer = '这是AI答案' WHERE id = ?",
                (qb["id"],),
            )
            conn.commit()
...
class TestFullRebuildSimulation:
    ...
    def test_5_interviews_rebuild(self, mock_uq, mock_cluster, mock_tag, mock_db):
        ...
        async def _run():
            ...
            assert conn.execute(
                "SELECT COUNT(*) as c FROM question_position"
            ).fetchone()["c"] == len(qb_rows)
