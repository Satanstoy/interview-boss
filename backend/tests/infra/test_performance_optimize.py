...
class TestLLMRetryLimit:
    ...
    def test_retry_has_delay_limit(self):
        ...
        assert "stop_after_delay" in content, "LLM 重试缺少 stop_after_delay 总时间限制"
...
class TestUserClientCacheLimit:
    ...
    def test_cache_has_max_size(self):
        ...
        # 检查是否有缓存大小限制逻辑
        assert (
            "MAX_CACHE" in llm_content
            or "max_cache" in llm_content
            or "cache_size" in llm_content
            or "len(_user_client_cache)" in llm_content
