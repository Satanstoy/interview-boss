...
`/mcp` 路径已在 `CSRFMiddleware` 中豁免，避免外部 MCP client 因缺少自定义头被拦截。`mcp_app` 外层包有 `MCPAuthMiddleware`：当环境变量 `MCP_API_KEY` 设置时，要求请求携带正确的 `X-MCP-API-Key` 头或 `mcp_api_key` 查询参数。
