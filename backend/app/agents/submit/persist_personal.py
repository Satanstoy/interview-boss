...
from app.agents.shared.events import emit_progress, emit_error, build_matching_data, NodeTimer
...
async def match_and_persist_personal_node(state: SubmitState) -> dict:
    ...
    with NodeTimer() as timer:
        ...
        def _load_existing_bank():
            with get_db_connection() as conn:
                ...
                personal_rows = conn.execute(
                    "SELECT id, question, cat2, sources, original_questions, original_question_sources FROM question_bank WHERE owner_id = ? AND job_position = ? AND duplicate_of IS NULL",
                    (user_id, current_pos)
                ).fetchall()
                # 公共题
        ...
        for r in existing_personal:
            ...
            try:
                ...
                pass
            personal_by_cat2[cat2].append({"question_bank_id": r['id'], "question": r['question'], "all_questions": all_qs})
        ...
        for r in existing_public:
            ...
            try:
                ...
                pass
            public_by_cat2[cat2].append({"question_bank_id": r['id'], "question": r['question'], "all_questions": all_qs})
        ...
        # Phase 1: 匹配个人题库
        personal_match = await match_new_questions(new_rows_for_match, personal_by_cat2, user_id=user_id)
        personal_matched_ids = {m["new_id"] for m in personal_match["matched"]}

        # Phase 2: 未匹配的再匹配公共题库
        unmatched_for_public = [r for r in new_rows_for_match if r['id'] not in personal_matched_ids]
        public_match = {"matched": [], "unmatched": unmatched_for_public}
        if unmatched_for_public and public_by_cat2:
            public_match = await match_new_questions(unmatched_for_public, public_by_cat2, user_id=user_id)

        # 合并结果
        # 公共匹配的题也需要写入个人库（带 duplicate_of），加入 unmatched 列表
        pub_dup_ids = {m["new_id"] for m in public_match["matched"]}
        pub_dup_rows = [r for r in unmatched_for_public if r['id'] in pub_dup_ids]
        truly_unmatched = public_match["unmatched"]

        match_result = {
            "matched": personal_match["matched"],  # 匹配个人题的走合并路径
            "unmatched": truly_unmatched + pub_dup_rows,  # 新题 + 公共匹配题（都需写入）
        }
        public_duplicates = public_match["matched"]  # 匹配公共题的标记 duplicate_of
        idx_to_row = {idx: r for idx, r in enumerate(valid_rows)}

        dup_msg = f"，{len(public_duplicates)} 道已收录公共库" if public_duplicates else ""
        new_msg = f"{len(truly_unmatched)} 道新题" if truly_unmatched else ""
        combined = "、".join(filter(None, [
            f"{len(personal_match['matched'])} 道已有题目" if personal_match['matched'] else "",
            new_msg,
        ])) or "0 道新题"
        emit_progress(state, "match",
        ...
        answer_tasks, interview_id = await run_db(lambda: submit_interview_txn(
            saved_url, data, "\n".join(f"{i+1}. {q}" for i, q in enumerate(data.get("具体题目清单", []))),
            season, record_owner_id, record_status, current_pos,
            tagged_rows, match_result["matched"], match_result["unmatched"],
            idx_to_row, bool(is_admin), user_id, qb_owner_id=user_id,
            public_duplicates=public_duplicates
        ))
    ...
    return {
        ...
        "node_timings": {**state.get("node_timings", {}), "match_persist": timer.elapsed},
    }
...
async def jd_persist_node(state: SubmitState) -> dict:
    ...
    with NodeTimer() as timer:
        ...
        tech_stack = "\n".join(f"{i+1}. {item}" for i, item in enumerate(tech_list)) if tech_list else "未提供"

        await run_db(lambda: _insert_jd(saved_url, data, tech_stack, season, owner_id=record_owner_id, status=record_status, job_position=job_position))
