...
def resolve_active_skills(state: ChatState) -> dict:
    """根据当前状态匹配 active skills，供策略和生成共用。"""
    ...
    strategy_rules = {
        skill.name: skill.get_strategy_rules()
        for skill in matched_skills
        if skill.get_strategy_rules()
    }
    logger.info(f"Active skills: {active_skill_names}")
    return {
        "active_skills": active_skill_names,
        "active_skill_strategy_rules": strategy_rules,
    }


def plan_skill_guided_strategy(state: ChatState) -> dict:
    """基于 active skills 的 strategy_rules 生成面试检索策略。

    真正消费 SKILL.md 中的 strategy_rules，而不是硬编码 skill 名称。
    """
    ...
    strategy_rules = state.get("active_skill_strategy_rules", {})
    history = state.get("message_history", [])
    assistant_questions = [m for m in history if m.get("role") == "assistant"]
    ...
    # 判断候选人回答质量（用于 adaptive-difficulty）
    answer_is_strong = answer_complete and len(user_message.strip()) > 100
    answer_is_weak = not answer_complete or len(user_message.strip()) < 30

    # 噪声检测（用于过滤误触发的 skill）
    _noise_patterns = ["比如", "例如", "举例", "像", "噪声", "不该", "不是", "排除"]
    is_noise_context = any(p in user_message for p in _noise_patterns)
    ...
    has_project_detail_signal = any(
        term in user_message.lower() for term in project_detail_terms
    )

    if intent in ("chat", "follow_up") or not answer_complete:
        strategy = "clarification"
        reason = "用户回答尚不完整或当前是追问/闲聊，先澄清而不是检索新题。"
        should_retrieve = False
        preferred = question_type or "project_followup"
    elif intent == "practice_request":
        strategy = "topic_shift"
        reason = "用户主动要求开始或换题，应切换到新的高频题方向。"
        should_retrieve = True
        preferred = question_type or "new_question"
    elif "project-deep-dive" in active_skills and question_count < 4:
        strategy = "deep_dive"
        reason = "项目相关 skill 激活且仍处于前几轮，应继续围绕项目实现深挖。"
        should_retrieve = True
        preferred = "project_followup"
    elif "project-deep-dive" in active_skills and question_count % 4 not in (0,):
        strategy = "deep_dive"
        reason = "项目细节仍有追问空间，保持 deep_dive，避免被示例噪声带偏。"
        should_retrieve = True
        preferred = "project_followup"
    elif (
        "project-deep-dive" in active_skills
        and has_project_detail_signal
        and question_count < 8
    ):
        strategy = "deep_dive"
        reason = "候选人最新回答仍包含具体项目实现信号，优先追问项目细节，不急于随机跳题。"
        should_retrieve = True
        preferred = "project_followup"
    else:
        strategy = "topic_shift"
        reason = "当前主题已追问多轮，按照 interview-rhythm 切换到新的高频方向。"
        should_retrieve = True
        preferred = question_type or "knowledge_probe"

        # 消费 strategy_rules 而不是硬编码
        for skill_name, rules in strategy_rules.items():
            if not isinstance(rules, dict):
                continue

            # 检查 noise_filter
            noise_filter = rules.get("noise_filter", "")
            if noise_filter and is_noise_context:
                reason += f" {skill_name} 因噪声上下文被跳过。"
                continue

            # 检查 topic_shift 规则
            topic_shift_rules = rules.get("topic_shift", {})
            if not topic_shift_rules:
                continue

            trigger = topic_shift_rules.get("trigger", "")
            preferred_topics = topic_shift_rules.get("preferred_topics", [])

            # 检查 trigger 条件
            if "target_topic" in trigger:
                topic_lower = search_query.lower() if search_query else ""
                topic_matches = any(t.lower() in topic_lower for t in preferred_topics)
                if topic_matches:
                    if skill_name == "adaptive-difficulty":
                        if answer_is_strong:
                            preferred = "knowledge_probe"
                            reason += (
                                " 候选人回答扎实，可提升难度到系统设计或工程权衡层面。"
                            )
                        elif answer_is_weak:
                            preferred = "new_question"
                            reason += " 候选人回答薄弱，切回基础高频题巩固。"
                    elif skill_name == "theory-qa":
                        preferred = "knowledge_probe"
                        reason += f" target_topic '{search_query}' 是基础知识，优先激活 theory-qa。"
                    elif skill_name == "algorithm-coding":
                        if not is_noise_context:
                            preferred = "new_question"
                            reason += f" strategy 决定切到算法题 '{search_query}'，不是 query 污染。"
                        else:
                            reason += f" target_topic '{search_query}' 是噪声示例，不激活 algorithm-coding。"

    # 清洗 target_topic：移除 negative_terms 中的词
    if strategy == "topic_shift" and negative_terms:
        for term in negative_terms:
            if term.lower() in (search_query or "").lower():
                search_query = search_query.replace(term, "").strip()
                reason += f" target_topic 已移除噪声词 '{term}'。"

    # 收集被抑制的 skills（噪声上下文下的 algorithm-coding / theory-qa）
    suppressed_skills = []
    if is_noise_context:
        for skill_name, rules in strategy_rules.items():
            if not isinstance(rules, dict):
            ...
            noise_filter = rules.get("noise_filter", "")
            if noise_filter:
                suppressed_skills.append(skill_name)

    if strategy == "deep_dive":
        target_topic = search_query or "项目实现细节"
        rerank_goal = (
            "选择能支撑项目细节追问的题目，优先 RAG、LangGraph、检索链路、"
            "RRF、LLM rerank、selected_basis、references、日志和失败 case。"
        )
        tool_policy = "retrieve_related" if should_retrieve else "none"
    elif strategy == "topic_shift":
        target_topic = search_query or state.get("job_position") or "岗位高频基础题"
        rerank_goal = (
            "选择岗位高频且适合自然转场的新主题；不得把 negative_terms 作为跳题目标。"
        )
        tool_policy = "draw_question"
    else:
        target_topic = "澄清上一轮回答"
        rerank_goal = "不选择新题，不展示 references。"
        tool_policy = "none"

    effective_skills = [s for s in active_skills if s not in suppressed_skills]

    result = {
        "strategy": strategy,
        "tool_policy": tool_policy,
        "strategy_reason": reason,
        "strategy_target_topic": target_topic,
        "strategy_preferred_question_type": preferred,
        "strategy_should_retrieve": should_retrieve,
        "strategy_rerank_goal": rerank_goal,
        ...
        "skill_decision": {
            ...
            "strategy": strategy,
            "tool_policy": tool_policy,
            "preferred_question_type": preferred,
            "target_topic": target_topic,
            "reason": reason,
        },
    }
    logger.info(
        "Skill-guided strategy: "
        f"active_skills={active_skills}, strategy={strategy}, "
        f"target_topic='{target_topic}', reason={reason}"
    )
    return result
