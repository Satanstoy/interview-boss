...
| `skills/builder.py` | 生成 lightweight catalog 与 active skill instruction prompt |
...
3. 修改 `skills/` 后运行 `docker compose --profile test run --rm test uv run pytest backend/tests/chat/test_chat_skills.py backend/tests/chat/test_skill_catalog.py -q`
4. 更新本文件
