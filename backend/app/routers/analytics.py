...
def _build_analytics_bank_filter(user: dict):
    """构建 analytics 查询的题库过滤条件（委托统一口径，默认 all）
...
async def get_analytics(user: dict = Depends(get_current_user)):
    ...
    def _query():
        ...
        with get_db_connection() as conn:
            ...
            position_filter = (
                "(job_position = ? OR job_position = '' OR job_position IS NULL)"
            )
            # 过滤口径：all = 公共 + 自己的（JD 表按 owner_id 过滤）
            ...
            for r in conn.execute(
                f"SELECT tech_stack FROM jd {jd_where}", jd_params
            ).fetchall():
                if r["tech_stack"]:
                    tech_counter.update(
                        [
                            ...
                            if t.strip()
                            and t.strip().lstrip("0123456789. ").lower()
                            not in _skip_tech
                        ]
                    )

            # 过滤 questions_detail 数据（通过关联 interview 表，all 口径）
        ...
        return (
            dict(tech_counter.most_common(10)),
            dict(tag_counter.most_common(10)),
            dict(tag_counter.most_common(20)),
            dict(level_counter),
        )
...
async def get_practice_stats(user: dict = Depends(get_current_user)):
    ...
    def _query():
        with get_db_connection() as conn:
            # 题库范围（all 口径：公共 + 自己的私有题）
            join_clause, bank_where, bank_params = _build_analytics_bank_filter(user)

            # Master bank difficulty distribution
            diff_counts = {}
            for r in conn.execute(
                f"SELECT qb.difficulty, COUNT(*) as cnt FROM question_bank qb {join_clause} {bank_where} GROUP BY qb.difficulty",
                bank_params,
            ).fetchall():
...
async def normalize_categories(admin: dict = Depends(get_admin_user)):
    ...
    def _normalize():
        ...
        with get_db_connection() as conn:
            cursor = conn.cursor()
            rows = cursor.execute(
                "SELECT id, cat1, cat2 FROM questions_detail WHERE deleted_at IS NULL"
            ).fetchall()
            for r in rows:
                ...
                if new_cat1 != r["cat1"] or new_cat2 != r["cat2"]:
                    cursor.execute(
                        "UPDATE questions_detail SET cat1 = ?, cat2 = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
                        (new_cat1, new_cat2, r["id"]),
                    )
                    from app.db.operations import _retype_distribution_details_txn

                    _retype_distribution_details_txn(cursor, [r["id"]])
                    updated_detail += 1
        ...
        with get_db_connection() as conn:
            ...
            try:
                ...
                positions = cursor.execute(
                    "SELECT DISTINCT job_position FROM interview WHERE owner_id IS NULL AND status = 'approved' AND deleted_at IS NULL"
                ).fetchall()
...
async def get_knowledge_graph(user: dict = Depends(get_current_user)):
    ...
    for tag, count in tag_counts.items():
        primary_cat = (
            tag_cat_counts[tag].most_common(1)[0][0]
    ...
    for (tag_a, tag_b), count in cooccurrence.items():
        if count >= 2:
            links.append(
                {"source": f"tag:{tag_a}", "target": f"tag:{tag_b}", "weight": count}
            )
