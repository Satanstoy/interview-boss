...
| `practice.py` | `/api/practice/*` | LeetCode 风格刷题队列、系统/自定义题单、题单题目管理与间隔复习。收藏/复习/加题单/evaluate-answer 的可见性统一 all 口径（`build_bank_where_clause(user_id, "all")`：公共题 + 自己的题），与题库列表一致。复习与自评记录复习时通过 `_user_urgency()` 按用户招聘偏好（`user_recruitment_pref` 届次 + pace）计算机会窗口 urgency 传入 `record_review`（无偏好 → 0.2 base）；`POST /api/practice/review` 从请求体透传可选 `idempotency_key`，重发同键不重复推进 SRS（audit D14） |
