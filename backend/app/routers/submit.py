...
from app.services.llm import client, _should_use_response_format, _extract_json, _call_llm_with_retry_messages, get_llm_client_for_user
...
async def tag_questions_batch(url: str, company: str, round_: str, questions: List[str], taxonomy_config: dict = None, user_id: int = None) -> List[List[str]]:
    ...
    _c, _m, _t, _bu = get_llm_client_for_user(user_id) if user_id else (client, LLM_MODEL, None, None)
    kwargs = dict(
        model=_m,
        messages=[
...
async def submit_data(
    ...
    try:
        ...
        _c, _m, _t, _bu = get_llm_client_for_user(user['id'])
        llm_kwargs = dict(
            model=_m,
            temperature=0.1,
        )
        ...
        if doc_type == "jd":
            ...
            await run_db(lambda: _insert_interview(saved_url, data, questions, season.strip(), owner_id=record_owner_id, status=record_status, job_position=current_pos))
...
对于面试轮次，请从内容中推断（如一面、二面、HR面等）。"""
                        try:
                            _rc, _rm, _rt, _rbu = get_llm_client_for_user(user['id'])
                            retry_kwargs = dict(
                                model=_rm,
                                messages=[
                            ...
                            retry_response = await _rc.chat.completions.create(**retry_kwargs)
                    ...
                    _, tagged_rows = await asyncio.gather(
                        retry_fill_fields(),
                        tag_questions_batch(saved_url, data.get("公司", "未提供"), data.get("面试轮次", "未提供"), q_list, taxonomy_config, user_id=user['id'])
                    )
