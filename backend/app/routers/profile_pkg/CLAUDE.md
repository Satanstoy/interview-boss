...
| `resume.py` | `/api/profile/resume`, `/api/profile/resume/text`, `/api/profile/resume/optimize`, `/api/profile/resume/optimization` | 简历上传/查询/删除、原文预览、SSE 优化（points → delta → done）与结果查询；`/optimize` 请求体用 Pydantic `OptimizeResumeRequest`（错误类型 422，上限 100 字符）；SSE 编排在 `resume_service.optimize_resume_event_stream`（路由只做 HTTP 感知） |
