...
【代表题选择标准（重要）】
- 从组内题目中**选取**最清晰、**最完整规范**的一道作为 representative（不要改写、不要生成新题面）
- 优先选：涵盖所有变体考察点、表述具体完整、不含面试现场口语（如"你刚才说的那个"）
- 不要选过于简略的题面（如"工作流和agent区别"这类缺少上下文细节的）
- representative 必须原样等于组内某道题（不能拼接、修改）"""
