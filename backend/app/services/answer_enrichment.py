...
3. 结构：单一问题是否使用 3–4 个三级标题，多子问或复杂架构题可用 4–6 个；是否匹配场景 A/B/C（算法题给可运行代码，题目指定语言时必须遵从、未指定时使用 Python；系统设计题给总体分层、选型依据、执行链路与故障边界；原理题给核心解释+记忆锚点+实用场景）。
4. 字数：篇幅应跟随问题复杂度。单一概念题通常 300–500 字；多子问、系统设计、架构、方案选型或需要说明端到端流程的工程题可用 600–1000 字。不能仅因复杂题超过 520 字判错，也不能用篇幅为由删掉显式子问。
5. 完整性：是否遗漏该题的核心考点。
...
10. 工程闭环：题目问系统如何设计、管理或选型时，是否只罗列组件，却没说每个组件解决什么问题、适用边界、端到端执行链路和关键失败防线；缺少任一决定答案成立的环节都应指出。
...
- 保持短句、大白话、可背诵；单一问题用 3–4 个三级标题，多子问或复杂架构题可用 4–6 个。
...
- 按题目复杂度控制篇幅：单一概念题通常 300–500 字；多子问、系统设计、架构、方案选型或需要说明端到端流程的工程题可用 600–1000 字。删掉重复和空话，但不得删掉题目明问的维度。
- 修订复杂工程题时，保留或补齐“总体分层/主方案 → 组件与选型依据 → 端到端执行链路 → 故障边界与防线”；这是覆盖检查，不是固定标题。
...
# 确定性字数上限：复杂工程题需要完成设计闭环，不能与单概念题共用 520 字硬上限。
_DEFAULT_MAX_ANSWER_LEN = 700
_COMPLEX_MAX_ANSWER_LEN = 1200
_COMPLEX_QUESTION_RE = re.compile(
    r"(?:系统设计|架构|生产环境|技术选型|方案比较|存储结构|数据流|生命周期|"
    r"怎么进行管理|如何管理|如何设计|怎么设计|组合使用|端到端|写入|检索|召回)",
    re.IGNORECASE,
)


def _answer_length_limit(question: str) -> int:
    """Return a generous hard ceiling; the prompt/critic own the softer target."""
    question = question or ""
    multi_part = question.count("？") + question.count("?") >= 2 or any(
        marker in question for marker in ("并说明", "分别", "以及", "同时", "又不能")
    )
    if multi_part or _COMPLEX_QUESTION_RE.search(question):
        return _COMPLEX_MAX_ANSWER_LEN
    return _DEFAULT_MAX_ANSWER_LEN
...
async def refine_answer(
    ...
    for _round in range(max_rounds):
        ...
        if critique.get("verdict", "PASS") == "PASS" or not issues:
            ...
            max_answer_len = _answer_length_limit(question)
            if len(current) > max_answer_len:
                issues = [
                    {
                        "problem": f"候选答案共 {len(current)} 字，超出本题确定性上限 {max_answer_len} 字",
                        "evidence": "字数校验（确定性规则）",
                    }
                ]
            else:
